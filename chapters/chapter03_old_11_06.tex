...
% In the view of the actual conditions under which the data were collected (trigger requiring coincidence of at least 3 particles, 
% T-o-F reconstructed from coincidence with a fast particle) perhaps the more straightforward 
...
% 
% As it will be shown later, the verification of the method is very promising. This motivates the HADES group to the perform the analysis of more exclusive observables in the next step the analysis of the spallation data.
% 
...
% the specific conditions present during this data taking has to be taken into account. They are:
% 
...
% \item For data selected as above creation of $dE/dx_{(TOF)} vs. momentum$ and 
...
% \end{itemize}
% 
% In the following the details and results of each step of this selection procedure will be shown.
%  \\
...
% The identification of H isotopes and charged pions among the data registered 
% in HADES during $p+Nb$ run at 3.5 $GeV$ encountered two significant difficulties 
% which are not present in the selection of the "conventional" leptonic or hadronic products of the interest of the HADES collaboration.
...
\begin{enumerate}
	...
	%	Each detected event comprised at least three charged particles. 	
% 	Among them both the primary as the secondary reaction products were possible
% 	It means that the single distribution spectra of primary reaction products are contaminated with the coincidences with both other primary particles as well
% 	with secondaries.
	...
	% 	The trigger conditions could be fulfilled by detection of both the reaction products originating from the target as well as by the secondary particles created in the elements of the setup.
...
% 	The lack of the START detector for the Time of Flight (TOF) measurement influences a lot the identification strength of the HADES detection system for the reaction products which are subject of the current investigation. In view of the missing start detector, the (TOF) of the selected particle could be reconstructed by the comparison to the (TOF) of the fastest identified particle of the event. But as long as the single distribution of reaction products are of interest the coincidences with other particles have to be disregarded. For this reason the identification 
...
% Since the particle identification in this kind of analysis has limited strength the selected distributions of particles 
% are superimposed on the misidentified particles forming the background distributions. For this reason the PID and background 
...
% rely on the beta values for single spectra. 
% Thus beta information is only used to define the $dE/dx$ identification cuts. The following methods had been applied for the identification of particles:
...
\node[proc,join] (B){HGeant};
\node [proc, join] (C) {Hydra};
\node [proc, join,fill=lcfree!25] (D){$dE/dx$ vs $\theta$ histogram};
\node [proc, below=of D,fill=lcfree!25] (E){$dE/dx$ vs $\theta$ histogram};
\node[proc, join,fill=lccong!25] (F){Division 2/1};
\node[proc, join,fill=lccong!25] (G){Efficiency};
\node[test,join=by free](t0){Mult dependent?};
% \node[term] (H) {end}

\node[proc,right=of A] (A1){model (e.g INCL++)};
\node[proc,join] (B1){HGeant};
\node [proc, join] (C1) {Hydra};
\node[test,join=by free](t1){M=HM?};
\node [proc, fill=lcfree!25] (D1){$dE/dx$ vs $\theta$ histogram};
\node[proc,fill=lcfree!25] (E1){$dE/dx$ vs $\theta$ histogram};
\node[proc, join,fill=lccong!25] (F1){Division 5/4};
\node[proc, join,fill=lccong!25] (G1){Efficiency};
\node[proc, densely dotted, it,above= of A1] (P1){Real generator needed};
\node[term,right=of C1] (A2){Hades data};
\node[term,join] (B2){Mult(HM) histogram};
\node[term] (C2){Mult(M) histogram};

\node [coord, left=of A]  (c1)  {}; \cmark{1}
\node [coord, right=of D]  (c2)  {}; \cmark{2}
\node [coord, left=of B1]  (c3)  {}; \cmark{3}
...
\draw [*->,lccong] (D.east) -- (c2) |- (F);
% \path (t0.south) to node [near start, xshift=1em] {$y$} (H);
